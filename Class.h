...
Vehicle(int l = 0, int cl = 0, int cw = 0, int x = 0, int y = 0, int s = 0, bool hc = false, COLORREF c = RGB(255, 255, 255),
    ...
    // 预测并绘制轨迹
    void predictAndDrawTrajectory(int laneHeight, int middleY, int predictionSteps, const vector<unique_ptr<Vehicle>> &allVehicles) const;

    // 检查变道是否安全
    bool isLaneChangeSafe(int laneHeight, const vector<unique_ptr<Vehicle>> &allVehicles) const;

    // 检查与前车距离
    void checkFrontVehicleDistance(vector<unique_ptr<Vehicle>> &allVehicles, int safeDistance);
    ...
    // 平滑变道函数
    virtual bool smoothLaneChange(int laneHeight, const vector<unique_ptr<Vehicle>> &allVehicles);
    // 获取安全距离（可被子类重写）
